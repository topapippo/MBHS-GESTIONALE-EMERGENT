...
import { Clock, Scissors, CheckCircle, ArrowLeft, MapPin, Phone, Mail, Star, MessageSquare, ChevronDown, ChevronUp, ArrowRight, Instagram, Facebook } from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#1a1a2e] text-white" data-testid="website-landing">
      ...
      <footer className="py-8 border-t border-amber-400/10">
        <div className="max-w-6xl mx-auto px-4 text-center">
          <img src="/logo.png?v=3" alt={config.salon_name} className="w-12 h-12 rounded-2xl mx-auto mb-3 border border-amber-400/20" />
          <p className="text-gray-500 text-sm font-semibold">{config.salon_name || 'BRUNO MELITO HAIR'}</p>
          <p className="text-gray-700 text-xs mt-2">{config.address}</p>
          <p className="text-gray-800 text-xs mt-4">&copy; {new Date().getFullYear()} {config.salon_name || 'Bruno Melito Hair'}. Tutti i diritti riservati.</p>
        </div>
      </footer>
