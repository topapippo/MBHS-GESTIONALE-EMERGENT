...
export default function BookingPage() {
  ...
  if (showBooking) {
    return (
      <div className="min-h-screen bg-[#FFF9F5]">
        <Toaster position="top-center" />
        <div className="bg-white border-b border-[#eee] py-4 px-4 sticky top-0 z-50">
          <div className="max-w-lg mx-auto flex items-center gap-3">
            <Button variant="ghost" size="icon" onClick={() => setShowBooking(false)} className="text-[#1a1a1a] hover:bg-[#f5f5f5] shrink-0" data-testid="booking-back-btn">
              <ArrowLeft className="w-5 h-5" />
            </Button>
            <div className="flex items-center gap-3">
              <img src="/logo.png?v=3" alt="MBHS Salon" className="w-9 h-9 rounded-lg" />
              <div>
                <h1 className="text-[#1a1a1a] text-sm font-black leading-tight">BRUNO MELITO HAIR</h1>
                <p className="text-[#999] text-xs">Prenota il tuo appuntamento</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="flex items-center justify-center gap-2 mb-6">
            {[1, 2, 3].map(s => (
              <div key={s} className="flex items-center gap-2">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${step >= s ? 'bg-[#C8956C] text-white' : 'bg-[#eee] text-[#999]'}`}>
                  {step > s ? <CheckCircle className="w-4 h-4" /> : s}
                </div>
                {s < 3 && <div className={`w-12 h-0.5 ${step > s ? 'bg-[#C8956C]' : 'bg-[#eee]'}`} />}
              </div>
            ))}
          </div>
          {step === 1 && (
            <div className="space-y-4">
              <h2 className="text-xl font-black text-[#1a1a1a]">Scegli i Servizi</h2>
              {loading ? <div className="flex justify-center py-8"><Clock className="w-6 h-6 text-[#C8956C] animate-spin" /></div> : (
              ...
              {formData.service_ids.length > 0 && (
                <div className="bg-white p-4 rounded-xl border border-[#C8956C]/30">
          ...
          {step === 2 && (
            <div className="space-y-4">
              ...
              <div className="flex gap-3">
                <Button onClick={() => setStep(1)} variant="outline" className="flex-1 border-[#ddd] text-[#555] hover:bg-[#f5f5f5] font-bold">Indietro</Button>
                <Button onClick={() => setStep(3)} className="flex-1 bg-[#1a1a1a] text-white hover:bg-[#333] font-bold">Continua <ArrowRight className="w-4 h-4 ml-2" /></Button>
              </div>
            </div>
          )}
          {step === 3 && (
            <div className="space-y-4">
              ...
              <div className="flex gap-3">
                <Button onClick={() => setStep(2)} variant="outline" className="flex-1 border-[#ddd] text-[#555] hover:bg-[#f5f5f5] font-bold">Indietro</Button>
                <Button onClick={handleSubmit} disabled={submitting} className="flex-1 bg-[#1a1a1a] text-white hover:bg-[#333] font-bold">
  ...
  // ==================== LANDING PAGE ====================
  return (
    <div className="min-h-screen bg-[#FFF9F5] text-[#1a1a1a]" data-testid="booking-welcome">
      <Toaster position="top-center" />

      {/* NAVBAR */}
      <nav className="fixed top-0 left-0 right-0 z-50 bg-white/90 backdrop-blur-md border-b border-[#eee] shadow-sm">
      ...
      {/* SALON GALLERY */}
      <section className="py-20 sm:py-28 bg-[#FFF9F5]">
        <div className="max-w-6xl mx-auto px-4">
          <div className="text-center mb-12">
            <p className="text-[#C8956C] font-black text-sm tracking-widest uppercase mb-3">Il Nostro Salone</p>
            <h2 className="text-3xl sm:text-4xl font-black text-[#1a1a1a]">Dove Nasce la Bellezza</h2>
          </div>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {[
              ...
              <div key={idx} className="relative rounded-2xl overflow-hidden aspect-square group shadow-md">
                ...
                <p className="absolute bottom-3 left-3 text-white font-black text-sm">{item.label}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-20 sm:py-28 bg-white">
        <div className="max-w-6xl mx-auto px-4">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div className="rounded-2xl overflow-hidden h-80 lg:h-96 shadow-lg">
              <img src={SALON_RECEPTION} alt="Il nostro salone" className="w-full h-full object-cover" />
            </div>
            <div>
              <p className="text-[#C8956C] font-black text-sm tracking-widest uppercase mb-3">Chi Siamo</p>
              <h2 className="text-3xl sm:text-4xl font-black mb-6 text-[#1a1a1a]">Dal 1983<br />con Passione</h2>
              <p className="text-[#555] leading-relaxed mb-6 font-bold">
              ...
              <p className="text-[#555] leading-relaxed mb-8 font-bold">
              ...
              <div className="grid grid-cols-2 gap-3">
                {["Dal 1983 nel settore", "Senza parabeni e solfati", "Colorazioni senza ammoniaca", "Cheratina e olio di argan"].map((item, idx) => (
                  <div key={idx} className="flex items-center gap-2">
                    <CheckCircle className="w-4 h-4 text-[#C8956C] shrink-0" />
                    <span className="text-sm text-[#444] font-bold">{item}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      <section className="py-20 sm:py-28 bg-[#FFF9F5]">
        <div className="max-w-6xl mx-auto px-4">
          <div className="text-center mb-12">
            <p className="text-[#C8956C] font-black text-sm tracking-widest uppercase mb-3">Recensioni</p>
            <h2 className="text-3xl sm:text-4xl font-black text-[#1a1a1a]">Cosa Dicono di Noi</h2>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {REVIEWS.map((review, idx) => (
              <div key={idx} className="bg-white border border-[#eee] rounded-2xl p-5 shadow-sm hover:shadow-md transition-shadow">
                ...
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 bg-[#F8C8D4]/30 rounded-full flex items-center justify-center">
                    <span className="text-[#C8956C] font-black text-sm">{review.name[0]}</span>
                  </div>
                  <span className="text-sm text-[#666] font-black">{review.name}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="py-20 sm:py-28 bg-white">
        <div className="max-w-6xl mx-auto px-4">
          <div className="text-center mb-12">
            <p className="text-[#C8956C] font-black text-sm tracking-widest uppercase mb-3">Tendenze P/E 2026</p>
            <h2 className="text-3xl sm:text-4xl font-black text-[#1a1a1a]">I Nostri Lavori</h2>
            <p className="text-[#666] mt-3 max-w-xl mx-auto font-bold">Lasciati ispirare dalle ultime tendenze Primavera Estate 2026.</p>
          </div>
      ...
      {/* CONTACT SECTION */}
      <section ref={contactRef} className="py-20 sm:py-28 bg-[#FFF9F5]">
        <div className="max-w-6xl mx-auto px-4">
          <div className="text-center mb-12">
            <p className="text-[#C8956C] font-black text-sm tracking-widest uppercase mb-3">Contattaci</p>
            <h2 className="text-3xl sm:text-4xl font-black text-[#1a1a1a]">Prenota il Tuo Appuntamento</h2>
            <p className="text-[#666] mt-3 font-bold">Siamo pronti ad accoglierti nel nostro salone.</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
            <a href="https://maps.google.com/?q=Via+Vito+Nicola+Melorio+101+Santa+Maria+Capua+Vetere" target="_blank" rel="noopener noreferrer"
              className="bg-white border border-[#eee] rounded-2xl p-5 hover:border-[#C8956C]/30 hover:shadow-md transition-all text-center">
              <MapPin className="w-6 h-6 text-[#C8956C] mx-auto mb-3" />
              <h3 className="font-black text-[#1a1a1a] text-sm mb-1">Indirizzo</h3>
              <p className="text-[#666] text-xs leading-relaxed font-bold">Via Vito Nicola Melorio 101<br />Santa Maria Capua Vetere (CE)</p>
            </a>
            <div className="bg-white border border-[#eee] rounded-2xl p-5 text-center">
              <Phone className="w-6 h-6 text-[#C8956C] mx-auto mb-3" />
              <h3 className="font-black text-[#1a1a1a] text-sm mb-1">Telefono</h3>
              <a href="tel:08231878320" className="text-[#666] text-xs hover:text-[#C8956C] transition-colors block font-bold">0823 18 78 320</a>
              <a href="tel:3397833526" className="text-[#666] text-xs hover:text-[#C8956C] transition-colors block mt-1 font-bold">339 78 33 526</a>
            </div>
            <a href="mailto:melitobruno@gmail.com" className="bg-white border border-[#eee] rounded-2xl p-5 hover:border-[#C8956C]/30 hover:shadow-md transition-all text-center">
              <Mail className="w-6 h-6 text-[#C8956C] mx-auto mb-3" />
              <h3 className="font-black text-[#1a1a1a] text-sm mb-1">Email</h3>
              <p className="text-[#666] text-xs font-bold">melitobruno@gmail.com</p>
            </a>
            <div className="bg-white border border-[#eee] rounded-2xl p-5 text-center">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Button onClick={() => setShowBooking(true)} className="bg-[#1a1a1a] text-white hover:bg-[#333] font-black text-base px-10 py-6 rounded-xl shadow-lg w-full sm:w-auto">
              <Scissors className="w-5 h-5 mr-2" /> PRENOTA ORA
            </Button>
            <Button onClick={openWhatsApp} className="bg-[#25D366] hover:bg-[#20bd5a] text-white font-black text-base px-10 py-6 rounded-xl shadow-lg w-full sm:w-auto">
              <MessageSquare className="w-5 h-5 mr-2" /> WHATSAPP
            </Button>
            <a href="tel:08231878320" className="w-full sm:w-auto">
              <Button variant="outline" className="border-[#C8956C] text-[#C8956C] hover:bg-[#C8956C]/5 font-black text-base px-10 py-6 rounded-xl w-full">
      ...
      {/* FOOTER */}
      <footer className="py-8 bg-white border-t border-[#eee]">
      ...
      <div className="fixed bottom-0 left-0 right-0 p-3 bg-white/95 backdrop-blur-md border-t border-[#eee] sm:hidden z-50 shadow-lg">
        <Button onClick={() => setShowBooking(true)} className="w-full bg-[#1a1a1a] text-white hover:bg-[#333] font-black py-5 rounded-xl">
          <Scissors className="w-5 h-5 mr-2" /> PRENOTA ORA
        </Button>
      </div>
    </div>
